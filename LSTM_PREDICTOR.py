...
class LSTM_PREDICTOR(Model):
    ...
    def train(self,dataN):
        ...
        print("Predicted:",self.experiment.predict(classDatas[100:self.config.SEQ_LEN+100],self.config.INPUT_SIZE))
    def predict(self):
        classDatas = self.dataToClassFunc(np.asarray(self.dataMemory[-self.config.SEQ_LEN:]),self.thresholding)
        print(classDatas)
        return np.asarray(self.experiment.predict(classDatas,self.config.INPUT_SIZE))[0]
