...
if __name__ == '__main__':
    ...
    for i, d in enumerate(data):
        ...
        if i < 0:
            ...
            m1.message = [np.expand_dims(np.mean(data[i - 14:i + 1]), axis=0), i - 1,
                          np.expand_dims(np.mean(financeData[i - 14:i + 1]), axis=0), testDataOriginal[i],
                          originalFinanceSignal[i]]
        else:
        ...
        hp.communicateALLAgents(modelsList, m1.senderId, sendingObjectList)
        print("time:", i)
        print("Investmenlist:", modelsList[N].get_agent_total_money_list())
        print("GeneralScores:", modelsList[N].getAgentScores())
        print("PeriodicScores:", modelsList[N].getPeriodicScoreTableAgents())

        if modelsList[N].getPeriodicScoreTableAgents() != {}:
            lstmdeciderLog.append(modelsList[N].getPeriodicScoreTableAgents()["lstm_decider"])
            majorityVotingLog.append(modelsList[N].getPeriodicScoreTableAgents()["majorityDecider"])
            mlpdeciderLog.append(modelsList[N].getPeriodicScoreTableAgents()["mlpDecider"])
